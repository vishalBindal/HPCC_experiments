...
void 
 CreatePlot(std::vector<std::vector<double>> &tput)
 {
  ...
  //  if(resolution==1)
   ...
   for(int i=0;i<n_flows;++i)
   {
    ...
    for(int j=0;j<n_points;++j)
        dataset.Add(obs_ns*(j+1)/1e6, tput[i][j]);
...
main (int argc, char *argv[])
{
  ...
  std::vector<std::vector<double>> tput(n_flows, std::vector<double>(n_points));
  ...
  for(int i=0;i<n_flows;++i)
  {
    ...
    while(file>>time_ns)
    {
      ...
      if(node == flow_node_ports[i].first && port == flow_node_ports[i].second)
        if(int(time_ns/obs_ns) < n_points)
          tput[i][int(time_ns/obs_ns)] += packet_size;
    }
    
    for(int j=0;j<n_points;++j)
      tput[i][j] = tput[i][j] *  8  / obs_ns;     // Convert to GBits/sec
  }
